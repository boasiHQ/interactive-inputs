...
func TestConfig_NewFromInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "failed - timeout must be positive integer",
			preRun: func() {
			},
			envMap: map[string]string{
				"INPUT_TITLE":           "What name should be given to the barista?",
			...
			expectedConfig: config.Config{},
			expectedOutput: "::error::The 'timeout' input (0) must be a positive integer\n",
			expectedError:  errors.ErrInvalidTimeoutValueProvided,
		},
		{
